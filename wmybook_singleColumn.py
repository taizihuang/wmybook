...
class Book:
    def __init__(self, articleFile, commentFile, ver=''):
...
本书版权归属于王孟源(https://blog.udn.com/MengyuanWang/article)，采用 \\href{https://creativecommons.org/licenses/by-nc-nd/4.0/legalcode.zh-Hans}{CC BY-NC-ND 4.0} 协议开源，供读者免费阅读。
...
王孟源博客镜像网址为 https://wmyblog.site 。本书网址为 https://book.wmyblog.site ，用于发布修订说明。
    ...
    def formatArticle(self, art_id):
        ...
        #link
        post = re.sub(r'<a\ +href="(.+?)".*?>(.+?)<.*?\/a>',r'\\href{\1}{\2}',post)
        post = re.sub(r'<a href="(.*?)">',r'\\href{\1}{链接}',post)
        post = re.sub(r'}{(http.?:.*?)',r'}{链接',post)
    ...
    def formatComment(self, art_id):
        ...
        for idx in df.index:
            ...
            if comment_date > "2017/12/01 00:00":
                comment = re.sub(r'(https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b[-a-zA-Z0-9()@:%_\+.~#?&\/\/=]*)',r'\\href{\1}{链接}', comment)
                reply = re.sub(r'(https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b[-a-zA-Z0-9()@:%_\+.~#?&\/\/=]*)',r'\\href{\1}{链接}', reply)
            else:
                comment = re.sub(r'<a\ +href="(.+?)".*?>(http)?(.+?)<.*?\/a>',r'\\href{\1}{链接}',comment)
                reply = re.sub(r'<a\ +href="(.+?)".*?>(http)?(.+?)<.*?\/a>',r'\\href{\1}{链接}',reply)
                pass
    ...
    def export(self, skip_list=[]):
        content = self.header()
        for id in self.article.id[::-1]:
            if id in skip_list:
                continue
            content += self.formatArticle(id)
            content += self.formatComment(id)
        content += self.footer()

        with open(f'wmybook_{self.version}_singleColumn.tex', 'w', encoding='utf8') as f:
            f.write(content)
...
if __name__ == "__main__":
    ...
    skip_list = ['180085085', '178109529', '177490083', '177276403', '168244348']
    Book('./data/article_full.pkl', './data/comment_full.pkl', version).export(skip_list)    
